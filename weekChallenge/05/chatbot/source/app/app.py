...
from model import device, SOP_GPT
...
QA_CKPT = MODEL_DIR / "SOP_GPT_qa.pt"   # Stage 2: Q&A 응답
...
def generate(req: GenerateRequest):
    ...
    out = gen_model.generate(
        ...
        stop_tokens=SENTENCE_STOP_TOKENS, temperature=0.8, top_k=40, repetition_penalty=1.3,
    )[0].tolist()
...
def chat(req: ChatRequest):
    ...
    return ChatResponse(answer=decode(out[len(ids):], itos).strip())
